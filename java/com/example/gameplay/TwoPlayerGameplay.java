...
public class TwoPlayerGameplay {
    ...
    private final Image player2Left = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/com/example/attackofthelivingmodules/assets/Sprites/Player_2_Left.png")));
    private final Image player2Right = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/com/example/attackofthelivingmodules/assets/Sprites/Player_2_Right.png")));
    private final Image player2LeftDamage = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/com/example/attackofthelivingmodules/assets/Sprites/Player_2_Left_Damage.png")));
    private final Image player2RightDamage = new Image(Objects.requireNonNull(getClass().getResourceAsStream("/com/example/attackofthelivingmodules/assets/Sprites/Player_2_Right_Damage.png")));
    
    // Classes for Functions
    private StageUp stageUp;
    ...
    private long lastPlayerBulletTimeP1 = 0;
    private long lastPlayerBulletTimeP2 = 0;
    private long lastEnemyBulletTime;
    private double enemySpeed = 1;
    private double initialPos = 20;
    ...
    public TwoPlayerGameplay(Scene scene, GameplayController controller, Audio audio) {
        this.controller = controller;
        movement = new Movement(playerBullets, enemyBullets, enemyList, powerUpList, playerList, deadPlayerList, controller);
        player1 = new Player(player1Left, player1LeftDamage, 200, 525, 3, 1, movement);
        player2 = new Player(player2Right, player2RightDamage, 400, 525, 3, 2, movement);
        playerList.add(player1);
        playerList.add(player2);
        controller.setLives1(player1.getLives());
        controller.setLives2(player2.getLives());
    ...
    private void update(long now) {
        ...
        enemySpeed = stageUp.initializeEnemies(initialPos, enemySpeed, movement);
        playerUpdates(now);
        shootBullet(now);
        spawnPowerUp(now);
    ...
    private void shootBullet(long now) {
        if (keysPressed.contains(KeyCode.SPACE) && now - lastPlayerBulletTimeP1 >= COOLDOWN && player1.isAlive()) {
            ...
            lastPlayerBulletTimeP1 = now;
            audio.playPlayerBulletEffect();
        }
        if (keysPressed.contains(KeyCode.ENTER) && now - lastPlayerBulletTimeP2 >= COOLDOWN && player2.isAlive()) {
            playerBullets.add(new Bullet(playerBullet,player2.getX() + player2.getWidth() / 2 - playerBullet.getWidth() / 2 , player2.getY(), -player2.getPlayerSpeed(), 0));
            lastPlayerBulletTimeP2 = now;
            audio.playPlayerBulletEffect();
        }
    ...
    private void playerMovement() {
        ...
        if (keysPressed.contains(KeyCode.LEFT)) {
            player2.setX(player2.getX() - player2.getPlayerSpeed());
            if (player2.isDamaged()) player2.setImage(player2LeftDamage);
            else player2.setImage(player2Left);
        } else if (keysPressed.contains(KeyCode.RIGHT)) {
            player2.setX(player2.getX() + player2.getPlayerSpeed());
            if (player2.isDamaged()) player2.setImage(player2RightDamage);
            else player2.setImage(player2Right);
        }
        ...
        if (keysPressed.contains(KeyCode.UP)) {
            player2.setY(player2.getY() - player2.getPlayerSpeed());
        ...
        if (player2.getX() < 0) {
            player2.setX(0);
        } else if (player2.getX() + player2.getWidth() > 650) {
            player2.setX(650 - player2.getWidth());
        }
        ...
        if (player2.getY() < 525) {
            player2.setY(525);
        } else if (player2.getY() + player2.getHeight() > 725) {
            player2.setY(725 - player2.getHeight());
        }
    }
}
